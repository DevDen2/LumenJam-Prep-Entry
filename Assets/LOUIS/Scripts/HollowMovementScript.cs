...
public class HollowMovementScript : MonoBehaviour {
    ...
    void Update () {
        GetInputs();
        if (controls && !isledgegrab && can_move) {
            CheckLedgeGrab();
            Flip();
            Walk(xAxis);
            Recoil();
    ...
    void CheckLedgeGrab() {
        ...
        RaycastHit2D hit1 = Physics2D.Raycast(transform.position + new Vector3(rayStartOriented, ledgeRay1), orientation, ledgeRayLength, groundLayer);

        if (hit1.collider != null) { return; }
        
        RaycastHit2D hit2 = Physics2D.Raycast(transform.position + new Vector3(rayStartOriented, ledgeRay2), orientation, ledgeRayLength, groundLayer);
    ...
    public IEnumerator LedgeGrabRoutine(Vector3 targetPosition) {
        ...
        rb.linearVelocity = Vector2.zero;
        StartCoroutine(PauseMovementCoroutine(0.2f));
        rb.gravityScale = 1;
    ...
    {
        can_move = false; // Disable movement
    ...
    void Flip()
    {
        if (xAxis > 0)
        {
            transform.localScale = new Vector2(1.851852f, transform.localScale.y);
        }
        else if (xAxis < 0)
        {
            transform.localScale = new Vector2(-1.851852f, transform.localScale.y);
